...
:   Generate a fake activity graph with randomized on and off states for each of the modules. This is used for testing by the 2web developers.

`--full-fake-graph`

:   Generate a fake activity graph where the status of all modules is always on. This is used for testing by the 2web developers.

`--rebuild-themes`

:   Force the themes to be rebuilt.
