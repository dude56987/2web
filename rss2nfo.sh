...
rss2nfo_update(){
	...
	echo "$rssSources" | while read -r rssSource;do
		...
		echo "$rssAsJson" | jq -c | tac | while read -r rssObject;do
			...
			if ! test -f "/var/cache/2web/downloads/rss/$showTitle/tvshow.nfo";then
				...
				# generate a tvshow.nfo file from the base json data
				{
					...
					echo "</tvshow>"
				} > "/var/cache/2web/downloads/rss/$showTitle/tvshow.nfo"
			fi
			# get the air date
			...
			if ! test -f "/var/cache/2web/downloads/rss/$showTitle/$airDateYear/s${airDateYear}e$episodeNumber - $episodeTitle.nfo";then
				...
				createDir "/var/cache/2web/downloads/rss/$showTitle/$airDateYear/"
				{
					#echo "<?xml version='1.0' encoding='UTF-8'?>"
					echo "<episodedetails>"
					...
					# end the nfo file
					echo "</episodedetails>"
				} > "/var/cache/2web/downloads/rss/$showTitle/$airDateYear/s${airDateYear}e$episodeNumber - $episodeTitle.nfo"
				# get the playback url
				playbackUrl=$(echo "$rssObject" | jq -r ".url")
				# generate a .strm file from the media found in the rss
				echo "$playbackUrl" > "/var/cache/2web/downloads/rss/$showTitle/$airDateYear/s${airDateYear}e$episodeNumber - $episodeTitle.strm"
			fi
