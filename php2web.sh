...
function resetCache(){
	...
	# remove web cache
	delete "$webDirectory/phps/"
	delete "$webDirectory/thumbnails/phps/"
	#
	delete "/var/cache/2web/generated/phpCache/"
}
################################################################################
function nuke(){
	webDirectory="$(webRoot)"
	...
	delete "$webDirectory/applications/"
	# remove application sums
	delete "/var/cache/2web/generated/appSums/"
	# remove sql data
	sqlite3 $webDirectory/data.db "drop table applications;"
	# remove widgets cached
	delete "$webDirectory/web_cache/widget_random_applications.index"
	delete "$webDirectory/web_cache/widget_new_applications.index"
	drawLine
...
if [ "$1" == "-u" ] || [ "$1" == "--update" ] || [ "$1" == "update" ] ;then
	...
	update
elif [ "$1" == "--parallel" ] || [ "$1" == "parallel" ] ;then
	lockProc "php2web"
	checkModStatus "php2web"
	update
